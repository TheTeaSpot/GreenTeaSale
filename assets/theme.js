...
theme.Filters = (function() {
  ...
  var selectors = {
    ...
    filterSelection: '[data-tag-group]',
  ...
  function Filters(container) {
    ...
    this.$collectionBaseUrl = window.collectionBaseUrl;
  ...
  Filters.prototype = _.assignIn({}, Filters.prototype, {
    ...
    _onFilterChange: function() {
      ...
      // remove the 'page' and 'tags' parameters
      ...
      for(var i = 0; i < filterValues.length; i++) {
        tagString = tagString + filterValues[i] + ((i == (filterValues.length-1)) ? '' : '+');
      }

      var redirectUrl = this.$collectionBaseUrl + '/' + tagString + selectors.mainContent;
    ...
    _getFilterValues: function() {
      var values = [];
      $.each(this.$filterSelect, function(i, el){
        if($(el).val() != 'Any') {
          values.push( $(el).val() );
        }
      });
...
theme.Product = (function() {
  function Product(container) {
    ...
    this.selectors = {
      ...
      servingPrice: '[data-serving-price]',    
    ...
    $('input[name=purchase_type]').change(function(){
      var selected = $("input[name=purchase_type]:checked");
      if (selected.val() == 'autodeliver') {
	$('.price-item-compare').hide()
      } else {
    	$('.price-item-compare').show()
      }
    });
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _initAddToCart: function() {
      $(this.selectors.productForm, this.$container).on(
        ...
        function(evt) {
          ...
          if (isInvalidQuantity && ! window.BOLD && BOLD.common && ! BOLD.common.eventEmitter &&
    ...
    _updateAddToCart: function(evt) {
      var variant = evt.variant;
      var comingSoon = $('select option[value="' + variant.id + '"]').attr('coming-soon');
      ...
      if (variant) {
        if (comingSoon) {
            ...
            this.$addToCart
              ...
              .attr('aria-label', theme.strings.comingSoon);
            $(this.selectors.addToCartText, this.$container).text(
              theme.strings.comingSoon
            );
    ...
    _updatePrice: function(evt) {
      ...
      var $servingPrice = $(this.selectors.servingPrice, $priceContainer);	    
...
console.log("UPDATE PRICE")
      ...
      if (comingSoon){
        ...
        if (variant.compare_at_price > variant.price) {
          ...
          //Template servings
          servingsValue = $('select option[value="' + variant.id + '"]').attr('servings-value')
          if (servingsValue){
          $servingPrice.html(
                "(" + theme.Currency.formatMoney(variant.price / servingsValue, theme.moneyFormat) + " per serving)"
          );
          } else {
              $servingPrice.html('')
          }
...
$(document).ready(function($) {
  ...
  (function() {
  var zoomBoxes = document.querySelectorAll('.js-zoom-enabled');

  zoomBoxes.forEach(function(image) {
    var imageCss = window.getComputedStyle(image, false),
      imageUrl = image.dataset.zoom,
      foto = image.firstElementChild;
    ...
    imageSrc.onload = function() {
      ...
      //image.style.paddingBottom = ratio*100+'%';
  ...
  var s3baseUrl = 'https://tts-wp-images.s3.amazonaws.com/';
  var regex = /https:\/\/theteaspot.com\/steep-it-loose\//g
  var wwwRegex = /https:\/\/www\.theteaspot\.com\/steep-it-loose\//g
  var blogImages = $('#shopify-section-article-template').find('img');
  $.each(blogImages, function(i, img) {
    var wordpressFragment = 'wp-content';
    var imageSrc = $(img).attr('src');
    var imageSrcset = $(img).attr('srcset');

    if( imageSrc.indexOf(wordpressFragment) != -1 ) {
      var newSrc = imageSrc
        .replace('https://theteaspot.com/steep-it-loose/', s3baseUrl)
        .replace('https://www.theteaspot.com/steep-it-loose/', s3baseUrl);
      $(img).attr('src', newSrc);
    }
    if(typeof imageSrcset != 'undefined' && imageSrcset.indexOf(wordpressFragment) != -1) {
      var newSrcset = imageSrcset
        .replace(regex, s3baseUrl)
        .replace(wwwRegex, s3baseUrl);
...
theme.init = function() {
  ...
  $('.tts-feature-slider .tts-feature-slider-slides').slick({
    slidesToShow: 1
  });
